...
class SavedQuery(models.Model):
    ...
    name = models.CharField(max_length=255, help_text="Descriptive name for this query")
    ...
    latitude = models.FloatField(
        default=0.0, help_text="Latitude of the query location (-90 to 90)"
    )
    longitude = models.FloatField(
        default=0.0, help_text="Longitude of the query location (-180 to 180)"
    )
    query_params = models.JSONField(
        default=dict, help_text="Query parameters including date range and preferences"
    )
    ...
    result_data = models.JSONField(
        null=True, blank=True, help_text="Results from the electional analysis"
    )
    ...
    def clean(self):
        ...
        if (
            self.permission == self.Permission.NAMED_GROUP
            and not self.shared_with.exists()
        ):
