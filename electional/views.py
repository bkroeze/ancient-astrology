...
class SavedQueryDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        if not obj.can_view(self.request.user):
            raise PermissionDenied("You don't have permission to view this saved query.")
        return obj
...
class SavedQueryUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_object(self, queryset=None):
        ...
        if not obj.can_edit(self.request.user):
            raise PermissionDenied("You don't have permission to edit this saved query.")
        return obj
    
    def get_success_url(self):
        """Redirect to the detail view after editing."""
        return reverse_lazy("electional:saved_query_detail", kwargs={"pk": self.object.pk})
...
class SavedQueryDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_object(self, queryset=None):
        ...
        obj = super().get_object(queryset)
        if not obj.can_edit(self.request.user):
            raise PermissionDenied("You don't have permission to delete this saved query.")
        return obj
    ...
    def get(self, request, *args, **kwargs):
        ...
        if saved_query.job_status in (
            ...
            try:
                ...
                saved_query.save(update_fields=[
                    'job_status', 'job_error', 'result_data', 'updated_at',
                ])
